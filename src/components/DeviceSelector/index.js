...
import { queryDTUAll } from '../../services/api';
...
export default class DeviceSelector extends Component {
  ...
  componentDidMount = async () => {
    const { data: list } = await queryDTUAll();
    this.setState({ list });
  }
    ...
    return (
      <Modal footer={null} onCancel={this.modalCancel} {...modalProps} {...props.modalProps} >
        <Table dataSource={list} rowKey="id" onRow={this.getRowProps} >
          ...
          <Column title="DTU" dataIndex="dtuName" />
          <Column title="设备名称" dataIndex="deviceName" />
          <Column title="设备ID" dataIndex="id" />
